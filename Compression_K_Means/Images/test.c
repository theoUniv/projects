...
float distance_RGB(Color lower, Color upper){
  int re = upper.r - lower.r;
  int ge = upper.g - lower.g;
  int be = upper.b - upper.b;

  return abs((re+ge+be)/3);
}
...
Color * get_K_colors_from_CLUT(int k, Color * CLUT, int min_distance){
  ...
  for(i=get_size_of_CLUT(CLUT)-1; counter<k; i--){
    if(distance_RGB(ColorToInt(CLUT[i-1]), ColorToInt(CLUT[i])) > min_distance){
...
void set_centroids_for_image_colours(Color * centroids, Color * image_representation, int nb_centroids, Image * im){
  ...
  for(crt=im->data,i=0;i<nb_pixels;i++, crt+=3){
    temp.r = *crt;
    temp.g = *(crt+1);
    temp.b = *(crt+2);
    // Mettre à jour la plus petite distance.
    nearest_centroid = get_nearest_centroid(centroids, nb_centroids, temp);
    temp.r_centroid = nearest_centroid.r;
    temp.g_centroid = nearest_centroid.g;
    temp.b_centroid = nearest_centroid.b;
    result[i] = temp;
    result[i].is_last = -1;
  }
  result[i].is_last = 1;
  ...
  for(i=0; i<nb_pixels; i++){
    image_representation[i] = result[i];
  }
} 
...
void k_means(Image * image, int nb_centroids, int min_distance_centroid, int nb_iteration){
  ...
  for(i=0; i<nb_iteration; i++){
    update_centroid_from_colours(list_of_centroids[i], color_table, image);
    update_colours_from_centroid(list_of_centroids[i], color_table, image);
    update_image_with_color_vector(image, color_table);
    printf("Iteration effectuee n°%d\n", i);
    sleep(0.5); // On attends 2 secondes

...
void print_CLUT(Color * CLUT){
  ...
  for(i=0; i<get_size_of_CLUT(CLUT); i++){
    printf("Couleur: %u, R=%u, G=%u, B=%d, FREQUENCE: %d, ISLAST = %d\n", i, CLUT[i].r, CLUT[i].g, CLUT[i].b, CLUT[i].frequence, CLUT[i].is_last);
  }
  printf("\n-------------------\n");
}
...
Color get_nearest_centroid(Color * centroids, int nb_centroids, Color color){
  int i;
  int min_dist = 0;
  for(i=0; i<nb_centroids-1; i++){
    if(distance_RGB(centroids[i], color) < distance_RGB(centroids[i+1], color)){
...
void set_centroids_for_image_colours(Color * centroids, Color * image_representation, int nb_centroids, Image * im){
  ...
  for(i=0;i<nb_pixels;i++){
    ...
    nearest_centroid = get_nearest_centroid(centroids, nb_centroids, image_representation[i]);
    //printf("Centroide le plus proche trouvé: %d %d %d\n", nearest_centroid.r, nearest_centroid.g, nearest_centroid.b);
...
void update_centroid_from_colours(Color *centroide, Color *image_colour_tab, Image *im) {
  ...
  // On fait la somme des couleurs pour le centroïde actuel.
  for (i = 0; i < nb_pixels; i++) {
    if (image_colour_tab[i].r_centroid == centroide->r && image_colour_tab[i].g_centroid == centroide->g && image_colour_tab[i].b_centroid == centroide->b) {
      accR += image_colour_tab[i].r;
      accG += image_colour_tab[i].g;
      accB += image_colour_tab[i].b;
      indice_temp++;
    }
  }
  printf("%d On pour centroide R%d, G%d, B%D\n", indice_temp, centroide->r, centroide->g, centroide->b);
  // Si aucun pixel n'a été trouvé avec ce centroïde, on ne change pas la couleur.
  if (indice_temp > 0) {
...
void update_colours_from_centroid(Color *centroide, Color * image_colour_tab, Image * im){
  ...
  for(i=0;i<nb_pixels;i++){
    if(image_colour_tab[i].r_centroid == centroide->r && image_colour_tab[i].g_centroid == centroide->g && image_colour_tab[i].b_centroid == centroide->b){
      ...
      image_colour_tab[i].r = temp_mean.r;
      image_colour_tab[i].g = temp_mean.g;
      image_colour_tab[i].b = temp_mean.b;
    }
  }
}
...
// Tentative de dithering
void dithering(Image *im, int seuil) {
    int i, j;
    GLubyte *crt;
    for(j=0; j<im->sizeY; j++) {
        for(i=0; i<im->sizeX; i++) {
            crt = &(im->data[(j*im->sizeX+i)*3]);
            GLubyte R;
            if(*crt>seuil){
              R = 255;
            } else{
...
void k_means(Image * image, int nb_centroids, int min_distance_centroid, int nb_iteration, int dithering_enabled){
  int i, k, l;
  ...
  Color * temp = malloc(nb_centroids*sizeof(Color));
  ...
  for(i=0; i<nb_iteration; i++){
    ...
    int counter_temp = 0;
    int temp_num = get_size_of_CLUT(build_CLUT(image));
    for(l=0; l<nb_centroids; l++){
      temp[l] = list_of_centroids[l];
    }
    ...
    printf("Voici le nombre de couleurs différentes dans l'image: %d\n", temp_num);
    for(l=0; l<nb_centroids; l++){
      if(list_of_centroids[l].r == temp[l].r && list_of_centroids[l].g == temp[l].g && list_of_centroids[l].b == temp[l].b){
        counter_temp++;
      }
    }
    if(counter_temp == nb_centroids) {
      printf("La liste de centroide n'évoluent plus\n");
      break;
    }
    if(temp_num <= nb_centroids){
    ...
    // ------------------
      ...
      sleep(3); // On attends 2 secondes entre chaque iteration
      end = clock(); // Fin du chronomètre
      cpu_time_used = ((double)(end - start)) / CLOCKS_PER_SEC; // Calcul du temps CPU utilisé
      printf("Temps utilisé pour l'itération %d : %f secondes\n", i, cpu_time_used);
    }
